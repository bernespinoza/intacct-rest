...
module IntacctRest
  module Endpoints
    class CreateCustomer
      ...
      # successful result.
      def self.call(customer:, results: DEFAULT_RESULT_FIELDS, config: IntacctRest.configuration, token_provider: nil)
        unless customer.is_a?(IntacctRest::Model::Customer)
          raise ArgumentError, 'customer must be an IntacctRest::Model::Customer'
        end

        result = IntacctRest::Post.call(customer, config: config, token_provider: token_provider)
